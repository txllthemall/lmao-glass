...
def virtual_environment(n, tint):
    ...
    return np.clip(base*(1-spill*.08)+tint[None,None,:]*(spill*.08),0,1)
...
def render_artwork(path:Path, scale:float, ox:float, oy:float):
...
def render_one(slug, cfg, preset):
    ...
    nz=np.full_like(gx,0.65); norm=np.sqrt(gx*gx+gy*gy+nz*nz)+1e-6
    nx=-gx/norm; ny=-gy/norm; nz=nz/norm
    tint=hexrgb(cfg['tint']); env=virtual_environment(n,tint)
    ...
    refr_px=preset['refractionDp']*DP_SCALE; edge=np.clip(q,0,1)
    ...
    ndl=np.clip(nx*lx+ny*ly,0,1); disp_edge=edge*(ndl**2); disp=preset['dispersionDp']*DP_SCALE
    ...
    opp=np.clip(-(nx*lx+ny*ly),0,1)*edge; fres=(1-np.clip(nz,0,1))**1.5*edge
    rgb=refr*(0.82+0.18*tint[None,None,:]); rgb += spec[...,None]*0.50 + fres[...,None]*0.12; rgb -= opp[...,None]*0.08
    shadow=ndimage.gaussian_filter(edge,preset['shadowBlurDp']*DP_SCALE)*preset['shadowAlpha']; rgb-=shadow[...,None]*0.05
    rng=np.random.default_rng(20260903); rgb += rng.normal(0,preset['noise'],(n,n,1)).astype(np.float32)*mask[...,None]; rgb=np.clip(rgb,0,1)
    ...
    shadow_rgba=Image.new('RGBA',(n,n),(0,0,0,0)); shadow_rgba.putalpha(sh.point(lambda p:int(p*0.16))); base.alpha_composite(shadow_rgba,(5,8))
    g=np.asarray(glyph,dtype=np.float32)/255; g[...,3]*=preset['glyphOpacity']; g[...,:3]=np.clip(g[...,:3]*(0.92+0.10*ndl[...,None]),0,1)
    base.alpha_composite(Image.fromarray((g*255).astype('uint8'),'RGBA'))
...
def backgrounds(size):
    bg=[]
    for name,color in [('white',(245,245,245)),('black',(14,14,16)),('gray',(120,124,130)),('color',(55,72,155))]: bg.append((name,Image.new('RGB',(size,size),color)))
    a=np.zeros((size,size,3),np.uint8); y,x=np.mgrid[0:size,0:size]
    a[...,0]=(70+60*np.sin(x/17)+30*np.cos(y/29)).clip(0,255); a[...,1]=(80+55*np.sin((x+y)/31)+25*np.cos(x/13)).clip(0,255); a[...,2]=(110+70*np.cos(y/21)+25*np.sin(x/9)).clip(0,255)
    bg.append(('complex',Image.fromarray(a))); return bg

def contact_sheet(outputs, cfgs):
    cell=190; icon=130; labels=34; cols=5; rows=len(outputs)
    sheet=Image.new('RGB',(cell*cols,rows*(cell+labels)),(232,232,234)); draw=ImageDraw.Draw(sheet); bgs=backgrounds(icon)
    for r,(slug,im) in enumerate(outputs.items()):
        small=im.resize((icon,icon),Image.Resampling.LANCZOS)
        for c,(bn,bg) in enumerate(bgs):
            canvas=bg.copy().convert('RGBA'); canvas.alpha_composite(small); x=c*cell+(cell-icon)//2; y=r*(cell+labels)+8
...
def source_geometry_sheet(outputs,cfgs):
    cell_w=360; cell_h=260; icon=188
    sheet=Image.new('RGB',(cell_w*2,cell_h*len(outputs)),(232,232,234)); draw=ImageDraw.Draw(sheet)
    for r,(slug,rendered) in enumerate(outputs.items()):
        source=render_artwork(artwork_path(slug),cfgs[slug]['scale'],cfgs[slug]['offsetX'],cfgs[slug]['offsetY']).resize((icon,icon),Image.Resampling.LANCZOS); out=rendered.resize((icon,icon),Image.Resampling.LANCZOS)
        for c,(label,im) in enumerate((('official artwork + optical placement',source),('Liquid Glass render',out))):
            x=c*cell_w+(cell_w-icon)//2; y=r*cell_h+42; canvas=Image.new('RGBA',(icon,icon),(118,122,128,255)); canvas.alpha_composite(im); sheet.paste(canvas.convert('RGB'),(x,y)); draw.text((c*cell_w+12,r*cell_h+10),label,fill=(30,30,32))
...
'''
    (ROOT/f'android-pack/src/main/res/mipmap-anydpi-v26/ic_{slug}.xml').write_text(v26); (ROOT/f'android-pack/src/main/res/mipmap-anydpi-v33/ic_{slug}.xml').write_text(v33)
...
def ensure_output_dirs():
    for d in [ROOT/'android-pack/src/main/res/drawable',ROOT/'android-pack/src/main/res/mipmap-anydpi-v26',ROOT/'android-pack/src/main/res/mipmap-anydpi-v33',ROOT/'android-pack/src/main/assets',ROOT/'generated/icons',ROOT/'generated/contact-sheets',ROOT/'variants/regular',ROOT/'artwork/masks',ROOT/'artwork/height',ROOT/'artwork/normals']: d.mkdir(parents=True,exist_ok=True)

def main():
    ensure_output_dirs(); cfgs=json.loads((ROOT/'android-pack/launcher-mappings/icons.json').read_text()); presets=json.loads((ROOT/'renderer/material-presets.json').read_text()); preset=presets['liquid_regular']; outs={}
    for slug,cfg in cfgs.items():
        im,mask,height,normal=render_one(slug,cfg,preset); outs[slug]=im; im.save(ROOT/f'generated/icons/{slug}_1024.png'); im.resize((512,512),Image.Resampling.LANCZOS).save(ROOT/f'variants/regular/{slug}.png'); save_maps(slug,mask,height,normal); android_xml(slug)
        fg=im.resize((432,432),Image.Resampling.LANCZOS); fg.save(ROOT/f'android-pack/src/main/res/drawable/ic_{slug}_foreground.png')
        glyph=render_artwork(artwork_path(slug),cfg['scale'],cfg['offsetX'],cfg['offsetY']).resize((432,432),Image.Resampling.LANCZOS); a=glyph.getchannel('A'); mono=Image.new('RGBA',(432,432),(255,255,255,0)); mono.putalpha(a); mono.save(ROOT/f'android-pack/src/main/res/drawable/ic_{slug}_monochrome.png')
    contact_sheet(outs,cfgs); source_geometry_sheet(outs,cfgs); print('generated',len(outs),'icons from official source cache')
